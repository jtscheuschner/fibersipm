...
InitStatus R3BFisipmCal2Hit::Init()
{
    ...
    fCalDatas = (TClonesArray*)fMan->GetObject("FisipmCal"); // = branch name in TTree
    if (!fCalDatas)
    {
		printf("Couldnt get handle on FI4 cal items\n");
		return kFATAL;
	}
    //fHitDatas = (TClonesArray*)fMan->GetObject("R3BFisipmHitData");
    //fCalibration = NULL; // care about that later
    FairRootManager::Instance()->Register("FisipmHit", "Land", fHitDatas, kTRUE);
...
void R3BFisipmCal2Hit::Exec(Option_t* option)
{
    ...
    for (Int_t i = 0; i < nMapped; i++){
      ...
      R3BFisipmCalData* mCalData2 = (R3BFisipmCalData*)fCalData2->At(j);
      //get the stuff
      UInt_t det_type = mCalData->GetDetector();
      ...
      new ((*fHitDatas)[fHitDatas->GetEntriesFast()])
...
void R3BFisipmCal2Hit::BasicCut(Float_t t1, Float_t t2, UInt_t sipm1, UInt_t sipm2){
  if(TMath::Abs(t1-t2)>fMaxT) return kFALSE;
}

//read-functions for mapping
 void R3BFibersipmCal2Hit::ReadMapping(){//TODO
  ...
  // Loading Fiber mappings
   ...
   if(mapping.good()){
     LOG(INFO) << "R3BFiSiPMMapped2Cal opening Mapping file : " << mappingfile << FairLogger::endl;
